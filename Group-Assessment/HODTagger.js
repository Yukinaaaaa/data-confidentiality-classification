...
function tagHOFields(){
    ...
    fs.readFile('homeowners.json', 'utf8', (err, data) => {
        ...
        for (let i = 0; i < customers.length; i++){
            ...
            /*
            saved = saved + JSON.stringify(new_object);
            fs.writeFile("taggedhomeowners.json", saved, (err) => {
                if (err) {
                    console.log(err);
                }
            });*/

            if (i <= Math.floor(customers.length / 2)) {
                saved = saved + JSON.stringify(new_object);
                if (i == Math.floor(customers.length / 2)) {
                    fs.writeFile("taggedhomeowners.json", saved, (err) => {
                        if (err)
                ...
                if (i == customers.length-1) {
                    fs.appendFile("taggedhomeowners.json", saved, (err) => {
                        if (err)
                            console.log(err);
                    });
